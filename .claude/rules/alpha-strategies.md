...
|---|---|---|
| `rules.py` | Owner-authored rule strategies (Phase 5 S4): a strict, canonical JSON `RuleSpec` that says when to *be* long/short (conjunctions of `left op right` over `high`/`low`/`close`, `sma`/`ema`/`bbands`/`rsi`/`atr`/`macd` operands, the neurotrader888 operands `hawkes` (float decay first, `FLOAT_FIRST`)/`vsa`/`runs_z`/`perm_entropy`/`cmma`/`vg_path` (`price`/`inverse` field)/`reversibility` (window >= 10) — every chart indicator except `rsi_pc1`, whose PCA lives in `alpha_research` — or constants; ops `> < >= <=`; both sides true = flat), evaluated statelessly over exactly `history` trailing bars (fixed window = PIT by construction; warm-up on the decision bar is a `DataError`, never a zero). `canonical_json` is the run-identity bytes the manifest carries. Bias-guarded in `tests/unit/test_rules_bias_guard.py`; chart parity in `tests/unit/test_rules_indicators_parity.py` | `RuleSpec`, `Condition`, `Operand`, `parse_rule_spec`, `rule_spec_from_json`, `canonical_json`, `spec_sha256`, `operand_series`, `evaluate_rules`, `trailing_ohlcv`, `rule_signal` |
