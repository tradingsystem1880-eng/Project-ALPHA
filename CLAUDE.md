...
- `alpha_study` → core/data/patterns/research only. It publishes strict immutable feature-lineage, event-table, separate factor-observation, closed operator-registration, D0-validation-reference, and D1-mandate-reference projections. Every authority-linked value is explicitly `not_checked` until the existing CLI/ControlStore verifies it; tables declare `authority: none`; event tables contain no realized outcomes; and mandates add no D1 reservation or launch authority. It owns no persistence, CLI command, UI, approval, D1/D2 transition, promotion, paper, broker, or order authority (see `.claude/rules/alpha-study.md`).
