...
- `alpha_study` → core/data/patterns/research only. It publishes strict immutable feature-lineage, event/factor tables, closed operator registration, existing-authority references, derived finding/mechanism/advisor/workspace projections, and one thin registered-double-bottom → `EventTableV1` adapter. The adapter calls the existing detector, binds exact artifact/dataset lineage, and uses its full causal availability clock; event-study inference is deferred. Every authority-linked value is `not_checked`; findings copy typed evidence; advisors are non-executable proposals; workspaces contain references only. It owns no persistence, CLI command, UI, approval, D1/D2 transition, promotion, paper, broker, or order authority (see `.claude/rules/alpha-study.md`).
