...
| `../frontend/src/panels/{runBrowserModel,jobProgress,v3Models,mlTearsheetModel}.ts` | Pure typed panel models for capability-scoped run selection, honest job timing/ETA, v3 lifecycle state, and ML diagnostics; colocated Vitest files own branch semantics | bounded render-state projections only; no metric or verdict composition |
