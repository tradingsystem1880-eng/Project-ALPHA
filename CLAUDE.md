...
- Full Python gate (run before every commit; mirrors CI `.github/workflows/ci.yml`):
  `uv lock --check && uv sync --locked && uv run ruff check . && uv run ruff format --check . && uv run lint-imports && uv run mypy packages apps tests && uv run pytest -q -m "not network" --cov --cov-report=term-missing && uv run python scripts/generate_web_openapi.py --check && uv build --all-packages` followed by reinstall/import smoke for all 11 built wheels (see CI for the exact module assertion).
...
| Module | Responsibility | Key public symbols |
|---|---|---|
| `server.py` | FastMCP instance + 42 bounded tools + `main()` (stdio): 12 retained generic tools and 30 typed v3 tools for project/version/experiment/stage, durable job/suite cancellation and reconciliation, evidence, chart/comparison, holdout-seal, AgentBrief, and ML plan/launch. No MCP tool reveals a holdout or places an order. | legacy actions/reads plus typed v3 control and evidence resources |
...
| `api/` | Thin JSON routers | existing run/job/activity/catalog/control/paper/candle/analytics/workspace routes plus `development.py` (projects, immutable setup, suites, holdouts, jobs, evidence, AgentBrief) and `ml.py` (worker readiness, exchanges, durable jobs, evaluations, tear sheets) |
| `api/models.py` | Strict stable JSON response contracts; OpenAPI source | Pydantic models including `ChartBundle`, native tear sheets, project lineage, stage/jobs, evidence, and ML exchange/evaluation contracts |
| `_invoke.py` | Background job runner: spawn `alpha` in its own process group, guard lease/pump initialization, keep descendant groups leased after leader exit, poll audited cancellation, tail stdout, parse run/session ids, and publish SSE with `Last-Event-ID` replay | `Job`, `launch`, `event_stream`, `list_jobs`, `cancel_job`, `JOBS`, `RUN_TYPE` |
...
| `_development.py` · `_ml.py` | Thin subprocess adapters for CLI-owned development/evidence state and isolated ML exchange/jobs | project/suite/evidence projections; worker status/launch/import/evaluation projections |
...
**Workstation v3 — professional strategy-development program** (implementation and offline release gates verified 2026-07-19) — **Implemented; offline release gate passed.** The program includes manifest/run-identity/artifact-contract v3 with immutable causal chart traces and v1/v2 readers; fresh-state OOS/holdout execution with matching scoped evidence; six curated linked desks with atomic v2→v3 layout migration; bounded native dark Python-authored tear sheets plus deterministic QuantStats-Lumi audit export; complete-path Kronos K-line/cone/distribution/calibration; a governed SQLite project/version/experiment/14-stage/attempt/holdout/job/evidence control plane; versioned bounded REST projections; 42 bounded MCP tools (12 retained generic + 30 typed v3) with `AgentBrief`; and a separately locked Qlib/LightGBM worker whose close-stamped predictions are validated and synchronously replayed across the frozen universe through ALPHA's canonical engine. Qlib replay metrics are authoritative for that execution, but full counterfactual model-refit validation remains explicitly unimplemented. All trading remains local `SANDBOX`; holdout reveal and order placement are unavailable to unattended agents. Exact gate evidence is recorded in the post-v2 audit and risk register. R-22 still blocks distribution; R-14's opt-in Binance network smoke and R-24's reviewed UTC-rollover sandbox soak remain pending operational acceptance.
