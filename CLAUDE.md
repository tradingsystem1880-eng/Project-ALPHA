...
- `alpha_study` → core/data/patterns/research only. It publishes strict immutable feature-lineage, event/factor tables, closed operator registration, existing-authority references, and derived finding/mechanism/advisor/workspace projections. Every authority-linked value is explicitly `not_checked`; findings only copy typed evidence; advisors are non-executable proposals; workspaces contain references, never raw data or mutable authority. It owns no persistence, CLI command, UI, approval, D1/D2 transition, promotion, paper, broker, or order authority (see `.claude/rules/alpha-study.md`).
