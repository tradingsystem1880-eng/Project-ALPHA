...
- `alpha backtest run SYMBOL [--strategy ts_momentum|ma_crossover|mean_reversion|breakout|kronos, --param name=value, ...params, snapshot]` — one fixed-param run → artifacts. `kronos` params (floats): `context/horizon/samples/temperature/top_p/top_k/min_edge/band`; model selection via `ALPHA_FORECAST_*` env (never params); the CLI auto-precomputes the signal cache (`data_dir/forecasts/<key>`) before entering the engine.
