...
- `alpha_study` → core/data/patterns/research only. It publishes strict immutable feature-lineage projections; every lineage is explicitly an unverified reference until the existing CLI/ControlStore verifies it. It owns no persistence, CLI command, UI, approval, D1/D2, promotion, paper, broker, or order authority (see `.claude/rules/alpha-study.md`).
