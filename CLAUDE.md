...
| `api/` | Thin JSON routers | `runs.py` (index/detail/equity/trades/forecast/tearsheet), `jobs.py` (launch/list/detail/SSE-stream/cancel), `catalog.py` (strategies/commands/symbols/apps), `candles.py`, `options.py`, `risk.py`, `screener.py`, `research.py`, `workspaces.py` |
...
| `_runs.py` | Filesystem reads over the run store | `query_runs`, `run_detail`, `equity_series`, `trades`, `forecast_series` (reads the cone `quantiles.parquet` q05/q50/q95 + `history.parquet`), `tearsheet_file` |
...
| `../frontend/` | SPA source (Vite + React + TS + Dockview + Lightweight Charts + uPlot + AG Grid + cmdk); excluded from ruff/mypy/pytest; `npm run build` → `static/app` | — |
