"""``alpha forecast run``: probabilistic Kronos outcome cone for one symbol.

Samples future OHLCV paths from a pretrained Kronos model (or ``--model fake``, the
deterministic offline double used by tests/demos) over the trailing ``--context`` bars,
and writes the outcome-cone artifacts under ``data_dir/forecast/<run_id>/``. Pre-cutoff
context windows get a loud pretrain-overlap warning + manifest flag (ADR-0009).
"""
...
from alpha_cli import _forecast, _runner
