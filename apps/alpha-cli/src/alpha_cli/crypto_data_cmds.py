...
from alpha_cli._crypto_analysis import (
    LegacyUnscopedEventError as _LegacyUnscopedEventError,
)
from alpha_cli._crypto_analysis import (
    comparison_frame as _comparison_frame,
)
from alpha_cli._crypto_analysis import (
    coverage_row as _coverage_row,
)
from alpha_cli._crypto_analysis import (
    create_feature as _create_feature_with_store,
)
from alpha_cli._crypto_analysis import (
    feature_input_names as _feature_input_names,
)
from alpha_cli._crypto_analysis import (
    feature_projection as _feature_projection,
)
from alpha_cli._crypto_analysis import (
    manifest_acquisition_scope as _manifest_acquisition_scope,
)
from alpha_cli._crypto_analysis import (
    normalized_member as _normalized_member,
)
from alpha_cli._crypto_analysis import (
    parquet_frame as _parquet_frame,
)
from alpha_cli._crypto_coverage import batch_digest as _coverage_batch_digest
from alpha_cli._crypto_coverage import batch_directory as _coverage_batch_directory
...
class BulkVolumeUnconfigured(DataError):
    """No external volume is configured at all.

    Distinct from every other bulk-volume failure (unmounted, wrong UUID, not
    writable), which mean something is *wrong* and must fail loud. Nothing is
    configured means there is simply nothing to describe, so the read-only
    projections answer honestly and empty — the way ``storage`` already does.
    """
...
def _bulk_store() -> CryptoBulkStore:
    ...
    if not settings.bulk_volume_uuid:
        raise BulkVolumeUnconfigured("crypto bulk volume UUID is not configured")
        ...
        _emit(
            {
                ...
                "automatic_fallback": False,
                "execution_authority": False,
                "canonical_next_action": "Configure the reviewed Expansion volume UUID.",
            },
            json_out=json_out,
        )
        return
...
def feature_create(
    ...
    expected = _feature_input_names(feature_name)
    if expected is None:
...
def _batch_digest(body: dict[str, object]) -> str:
    return _coverage_batch_digest(body)


def _batch_directory(batch_id: str) -> Path:
    return _coverage_batch_directory(_coverage_batch_root(), batch_id)


def _write_batch_json(path: Path, value: dict[str, object]) -> None:
    _write_coverage_batch_json(path, value)
...
def _create_coverage_batch(
    ...
    *,
    ...
    return _create_coverage_batch_at(
        _coverage_batch_root(),
        profile,
        cadence=cadence,
        offset=offset,
        limit=limit,
...
def _read_coverage_batch(batch_id: str) -> tuple[dict[str, object], dict[str, object]]:
    return _read_coverage_batch_at(_coverage_batch_root(), batch_id)
