...
def load_bars(
    ...
    *,
    ...
    """Load the point-in-time history for ``symbol`` from the CLI store (``data_dir/store``).
    ...
    its window nor a split not yet known. The ``snapshot_id`` is recorded for provenance. Fails loud
    (``DataError``) on fewer than 2 bars.
    """
