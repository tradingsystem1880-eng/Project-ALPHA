...
| `alpha_cli/_synth.py` — bar-permutation gauntlet tier | Masters, *Testing and Tuning Market Trading Systems* (Apress 2018), ch. 7 for the permutation-test procedure and `(1 + c)/(1 + N)` (Davison & Hinkley 1997 ch. 4); the walk-forward construction (permute only the out-of-sample bars, re-run the fixed rules) is upstream neurotrader888/mcpt@2c0d70c `walkforward_donchian_mcpt.py`, which passes `start_index=train_window` (first OOS bar kept); ALPHA passes the decision bar before the first scored OOS bar so every scored bar is permuted — a recorded deviation, not a literature claim |
