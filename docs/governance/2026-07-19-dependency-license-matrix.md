...
| DefiLlama | Chain TVL (`defi_tvl`, supplemental research only; ADR-0036) | Keyless public API, one bounded request per chain-day series; free for non-commercial use with attribution recorded in every receipt; raw bytes retained locally for private research only | Terms change or removal request disables acquisition, then the inventory operation tombstones dependent snapshots; the derived `defi_tvl_residual` keeps input lineage |
...
| neurotrader888 repositories (TechnicalAnalysisAutomation, mcpt, market-structure, TrendLineAutomation, TrendlineBreakoutMetaLabel, VolatilityHawkes, VSAIndicator, RSI-PCA, TradeDependenceRunsTest, TimeSeriesReversibility, TimeSeriesVisibilityGraphs, PermutationEntropy, TVLIndicator) | [MIT](https://github.com/neurotrader888/TechnicalAnalysisAutomation/blob/main/LICENSE) per repository plus owner-recorded author approval (2026-09-12); IntramarketDifference has no licence | Pattern geometry, market-structure, volatility/volume indicators, complexity measures, permutation tests, PIP mining; upstream is pandas/matplotlib teaching code without tests | **Ported, never vendored** per `2026-09-12-neurotrader888-provenance.md`; no runtime dependency (pyclustering/ts2vg/networkx/pandas_ta/sklearn replaced by numpy); the unlicensed repository is re-implemented from its published formula only |
