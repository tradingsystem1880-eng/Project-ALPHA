...
| R-59 | Ticker collisions, token migrations, wrapped assets, or reused provider symbols join unrelated crypto histories | High | High | `CryptoAssetIdentityV1`; network+contract joins or reviewed native mapping only; point-in-time lifecycle/migration lineage; ticker-only joins fail | Collision, migration, relaunch, wrapper, delisting, and future-membership poison tests | Crypto data owner — **ADR-0032 gate** |
| R-60 | USD, USDT, USDC, linear/inverse contracts, or provider-specific quantities are silently treated as equivalent | High | High | Dataset identity binds quote asset, market type, units, contract type, multiplier, and timestamp convention; no universal candle or fallback; provider-native values remain unchanged | Cross-quote, inverse/linear, volume/OI unit, basis, funding-interval, and serialization tests | Crypto data owner — **ADR-0032 gate** |
| R-61 | DEX wash trading, manipulated liquidity, honeypots, or thin pools are promoted as reliable price evidence | High | High | GeckoTerminal remains pool-specific; liquidity/age/cadence/price-impact warnings; tracked/case-bound acquisition; DEX observations never automatically substitute for CEX execution prices | Synthetic wash/thin/stale/crossed pool fixtures, liquidity discontinuity, contract identity, and no-substitution tests | Crypto data/research owners — **ADR-0032 gate** |
| R-62 | Provider revisions or archive corrections overwrite evidence and make prior research unreplayable | Medium | High | Exact raw receipt/checksum; content-addressed immutable blobs; correction lineage; unexplained changes quarantine; snapshots bind ordered input hashes | Upstream byte-drift, checksum, correction, replay, tamper, and V1-compatibility tests | Crypto data owner — **ADR-0032 gate** |
| R-63 | Expansion fills, disappears, is substituted, or fails mid-download/publication and corrupts control state | Medium | High | Pin volume UUID; verify writable plus 15% and 100 GB reserve; bind partial Binance archives to URL and official checksum; accept only an exact HTTP Range continuation; keep completed downloads in an explicitly disposable checksum cache; atomically publish external bytes then internal manifest; control state stays internal | Missing/wrong UUID, low-space, read-only, interrupted range resume, ignored-range denial, tamper, and safe-cache-clean tests | Storage/operations owner — **Offline range-resume path verified 2026-08-15; live interruption evidence pending** |
| R-64 | Provider availability, rate limits, terms, or permitted retention change while UI continues to claim readiness | Medium | High | Family-level capability contracts; explicit receipted checks; no probe on page load; access tier/retention/attribution in manifests; terms/removal event blocks acquisition and invalidates dependent qualification; keyless DEX pagination is paced with bounded 429 backoff and resumable checkpoints expose safe blockers | 401/403/429/schema-drift, paced pagination, bounded retry, catalog removal, tombstone, stale-receipt, secret-redaction, and honest-state UI tests | Provider/governance owner — **ADR-0032 gate** |
