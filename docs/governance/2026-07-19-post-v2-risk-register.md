...
| R-63 | Expansion fills, disappears, is substituted, or fails mid-publication and corrupts control state | Medium | High | Pin volume UUID; verify writable plus 15% and 100 GB reserve; bounded resumable staging; atomically publish external bytes then internal manifest; control state stays internal | Missing/wrong UUID, low-space, read-only, interrupted publication/resume, tamper, and safe-cache-clean tests | Storage/operations owner — **ADR-0032 gate** |
