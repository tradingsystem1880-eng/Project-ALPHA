...
| TimeSeriesReversibility @ 3d76b9e | `reversibility.py` | `alpha_patterns/reversibility.py` | HVG from `alpha_patterns.visibility` (the author's `ts_to_vg` rule) instead of ts2vg; KL in numpy; embedding dimension parameterised; `async_index` breaks degree ties with a stable argsort (upstream's default argsort is platform-dependent: its tie order differs between x86 and arm64 numpy, so the pinned RAI values are ALPHA's own stable-sort reference, not upstream's); exact parity on sine / logistic / price windows for `perm_ts_reversibility` |
