...
**Scope:** D2 runner + governed admission of confirmation approval/D2 transitions, edge-validation
...
apps/alpha-cli/src/alpha_cli/control_store.py       # MODIFY: admit confirmation approval + D2 transitions (cites ADR-0026);
...
## Tasks
- [x] **D2 executor** — failing tests first: requires approved child confirmation contract +
...
- [x] **Un-disable (scoped)** — confirmation approval + D2 transitions admitted; every existing
      negative test that should still fail (agent actors, wrong boundary, early CONTRADICTED)
      re-asserted; commit cites ADR-0026.
- [x] **Checklist + full scorecard** — decision view (CLI `--json` + REST + decision tab):
      ...
      pytest).
- [x] **Promotion packet** — recorded atomically inside the `advance_to_strategy` decision
...
- [x] **`research_gate_state`** — derivation {not_required (grandfathered/legacy), open, passed,
      overridden} on project projections (CLI/REST/MCP `get_project`); tests per state.
- [x] **Override** — `alpha project override-research-gate` (owner CLI): append-only scope event
...
- [x] **UI gating** — StrategyLab/DevelopmentCenter/Pipeline disable strategy-creation and
...
- [x] **Program acceptance** — the full 2026-08-06 §13 + spec-§17 suite: end-to-end
