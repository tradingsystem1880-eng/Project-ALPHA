...
* **P3b** — the report toolbar is `Export CSV · Save PNG · Compare… · Run again · Notes` plus a
  ...
  re-taken.
* **P4** — `ownerStep(researchCase)` is derived from the case's own phase/review/execution/
  responsibility fields and offers exactly one step: `launch_d1` in the pilot and deep-research
  phases once exploration is approved, `launch_d2` for an approved sealed confirmation,
  `record_final_disposition` in `research_decision`, `revise_exploration` while material answers
  are owed (the choice cards became radios; a revision binds to the frozen source pack and is
  disabled with that reason until one exists), and the existing approve/reject boundary for a
  pending review. Every button is `OwnerActionButton` → the unchanged challenge/perform routes;
  the server re-validates, so a wrongly offered step fails as an honest 4xx, never as authority.
  `tests/unit/test_web_owner_actions_drift.py` pins the SPA vocabulary to
  `OwnerActionChallengeRequest.action_type` with `record_semantic_event` as the one documented
  exception. CLI-only steps hand over a `CopyCommand` naming the ADR: credential recovery
  (ADR-0030), the reviewed asset master (ADR-0032), the holdout reveal ceremony (ADR-0014,
  the previewed suite steps joined with `&&`). The Strategy Lab lock notice points at the
  cockpit's step rather than duplicating Touch ID; Governance › Touch ID links to
  `/owner-auth/enroll`. Touch ID itself is not automated: e2e proves the challenge body up to the
  WebAuthn prompt with the harness's deterministic credential.
