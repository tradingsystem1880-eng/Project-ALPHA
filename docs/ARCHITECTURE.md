...
| `alpha_study` | 2 — research composition | Strict immutable feature-lineage, event-table, and separate factor-observation projections plus a closed Git-owned operator declaration and exact `not_checked` references to existing D0 validation and frozen D1 inputs. Event tables exclude realized outcomes; mandate references create no D1 reservation or launch authority; the package owns no persistence, commands, UI, approvals, empirical evidence, D1/D2 transitions, promotion, paper, broker, or order authority. | `core`, `data`, `patterns`, `research` |
