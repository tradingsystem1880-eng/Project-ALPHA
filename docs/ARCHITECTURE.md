...
| `alpha_study` | 2 — research composition | Strict immutable feature-lineage projections over declared research-artifact references. S3a1 lineage is explicitly unverified until checked through the existing authority seam; the package owns no persistence, commands, UI, approvals, empirical evidence, D1/D2, promotion, paper, broker, or order authority. | `core`, `data`, `patterns`, `research` |
