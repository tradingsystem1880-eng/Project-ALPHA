...
| `alpha_study` | 2 — research composition | Strict immutable feature-lineage, event-table, and separate factor-observation projections over declared research-artifact references. S3a lineage is explicitly unverified and tables declare no authority until checked through the existing authority seam. Event tables exclude realized outcomes; the package owns no persistence, commands, UI, approvals, empirical evidence, D1/D2, promotion, paper, broker, or order authority. | `core`, `data`, `patterns`, `research` |
