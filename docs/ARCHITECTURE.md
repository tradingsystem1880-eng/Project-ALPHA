...
| `alpha_study` | 2 — research composition | Strict immutable feature/event/factor contracts, a closed Git-owned operator declaration, exact existing-authority references, and derived finding/mechanism/advisor/workspace projections. Findings copy typed evidence; graph relations and advisor actions are closed; workspaces contain typed references only. The package owns no persistence, commands, UI, approvals, empirical authority, D1/D2 transitions, promotion, paper, broker, or order authority. | `core`, `data`, `patterns`, `research` |
