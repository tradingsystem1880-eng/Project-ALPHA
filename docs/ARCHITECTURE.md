...
| `alpha_study` | 2 — research composition | Strict immutable feature/event/factor contracts, a closed Git-owned operator declaration, exact existing-authority references, derived finding/mechanism/advisor/workspace projections, one thin existing-double-bottom event-table adapter, and an S5a1 byte-bound blind semantic-read contract. The semantic contract emits only chart points available by its D0 acceptance-event reference and an aggregate masked count; its lineage remains unchecked until CLI composition. No inference or server/UI semantic bridge exists. The package owns no persistence, commands, UI, approvals, empirical authority, D1/D2 transitions, promotion, paper, broker, or order authority. | `core`, `data`, `patterns`, `research` |
...
- **Semantic owner events are a protected v5 ledger.** `record_semantic_event` is the one additional closed action: the existing owner-auth challenge/perform routes call the dedicated `alpha_cli.owner_auth` transaction, which binds one fresh assertion to one receipt and one canonical definition/review/freeze event. The v4 backup and lossless receipt `CHECK` rebuild are verified before the v5 marker; protected-object defects fail closed, and post-commit recovery is read-only. There is no direct semantic-write CLI command, new route, screen, frontend authority, MCP capability, or S5c presentation; committed-v5 forensic recovery requires owner approval and a forward migration. → [ADR-0035](adr/0035-generic-study-composition-and-external-capability-adapters.md)
