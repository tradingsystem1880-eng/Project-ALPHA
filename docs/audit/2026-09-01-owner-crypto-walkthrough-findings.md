...
| 14 | UX | Web · Market Watch | `BTC/USDT` and `BTC/USD` read as the same asset listed twice, and `93,354.22` (the stored 2026-06-30 close) read as today's BTC price; expected one row per stored pair naming its venue and the date of every price. | Owner screenshots 2026-09-03 | fixed `d823961` (venue + Age columns, `?tail=2` reads), `efe3322` (opt-in public ticker, `live` age) |
| 15 | UX | Web · shell | The Phase 3 chrome (warning glyphs, Inter 11px, top dock tabs, card Toolbox, emoji buttons) did not match the approved artboards in `ALPHA-terminal-designs/`. | Owner screenshots 2026-09-03 vs `1-Terminal.png` … `4-Governance-window.png` | fixed `8d72d3c` `ab3bda0` `fee839e` (icon toolbar, dock frames with bottom tabs, document header, Toolbox table, status bar wording, report toolbar/tree, Governance Item·State·Detail, full-window figure overlay) |
| 16 | UX | Web · Research | Every "owner approval / Touch ID / trusted CLI" notice was a dead end although the owner-auth routes already dispatch `launch_d1`, `launch_d2`, `revise_exploration`, `record_final_disposition`. | Cockpit `NEXT · OWNER`, unavailable choice cards, holdout notice | fixed `7acf3eb` (`OwnerActionButton` on every owner step, inline material-question answers, decision form; `Copy command` + ADR reason where CLI-only) |
...
### 2026-09-04 — Phase 4 "Pixel" delivered

P1–P5 of `docs/superpowers/plans/2026-09-04-trader-terminal-phase4-pixel.md` are committed behind the full gate on `feat/trader-terminal-phase1-work` (`d823961`, `efe3322`, `8d72d3c`, `ab3bda0`, `fee839e`, `7acf3eb`, plus the fix and docs commits). Findings #14–#16 (raised by the owner's rejection of Phase 3 on 2026-09-03) are closed. A scripted real-backend acceptance in both profiles at 1585×991 and 1440×900 then found and fixed two more honesty defects the mocked harness could not show: legacy datasets whose provenance reads `unknown` were labelled `Unknown` as if it were a venue (now `—`), and float32 closes rendered as `62.029999` (now rounded to seven significant digits). Touch ID itself cannot be scripted: the D1/D2/revise/decision buttons are proven up to the WebAuthn prompt, and the owner completes one on a real case.
