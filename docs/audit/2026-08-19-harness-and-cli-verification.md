...
| P4 | `rm -rf /tmp/never-exists-alpha-probe` | `pre-bash-guard` | exit 0 at probe time (`/tmp/` was then in `_SCRATCH_PREFIXES`). PR #42's first CI run showed the Linux consequence — recursive rm of pytest tmp dirs allowed — so the bare `/tmp/` prefix was **removed** (scratchpads stay allowed via `/tmp/claude-` and `/private/tmp/claude-`); the probe now BLOCKS | FIXED → BLOCKS |
