...
def test_read_only_projections_answer_honestly_when_no_volume_is_configured(
    ...
    ``storage`` already answered this way. Before this, the other four read-only
    projections raised instead, which surfaced as an HTTP 422 on parameter-free
    GETs — the Workstation reporting a bad request when the drive was simply
    never configured.
    """
    ...
    cases = (
        ...
        ("coverage", "canonical_next_action", {"items": [], "count": 0}),
        ("features", "next_action", {"items": [], "count": 0}),
        (
            "storage-inventory",
            "next_action",
            {"manifest_count": 0, "snapshot_count": 0, "cache_bytes": 0, "staging_count": 0},
        ),
    )
    for command, action_key, expected in cases:
        result = runner.invoke(app, ["crypto-data", command, "--json"])
...
def test_read_only_projections_still_fail_loud_when_storage_is_unavailable(
    ...
    """Unconfigured is empty; unavailable is wrong. Only the first answers quietly."""

    def blocked(*_args: object, **_kwargs: object) -> object:
        raise DataError("fixture storage is unavailable")

    monkeypatch.setattr(crypto_data_cmds, "_bulk_store", blocked)
    for command in ("capabilities", "coverage", "features", "storage-inventory"):
        result = runner.invoke(app, ["crypto-data", command, "--json"])
        assert result.exit_code != 0, command
        assert "fixture storage is unavailable" in result.output, command
